...
def index():
    ...
    if lesson_id:
        ...
        if selected_lesson:
            # Garante registros de presença para todos os alunos da turma
            klass = selected_lesson.klass
            existing_ids = {a.student_id for a in Attendance.query.filter_by(lesson_id=lesson_id).all()}
            new_records = [
                Attendance(lesson_id=lesson_id, student_id=s.id, present=False)
                for s in klass.students if s.id not in existing_ids
            ]
            if new_records:
...
@attendance_bp.route('/bibles', methods=['POST'])
@login_required
def bibles():
    lesson_id = request.form.get('lesson_id', type=int)
...
@attendance_bp.route('/magazines', methods=['POST'])
@login_required
def magazines():
    lesson_id = request.form.get('lesson_id', type=int)
    value     = int(request.form.get('value', 0) or 0)
    lesson    = Lesson.query.get_or_404(lesson_id)
    lesson.magazines = value
    db.session.commit()
    return jsonify({'ok': True})
