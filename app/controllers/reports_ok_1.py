...
def pdf():
    ...
    if current_user.congregation and current_user.congregation.church:
        ...
        # superadmin pode ter filtrado uma congregação específica
        if congregation_id:
    ...
    # Filtros secundários (turma e lição)
    filter_parts = []
    if class_id:
    ...
    C_CARD3=colors.HexColor('#0d1420')
    ...
    sHL = ParagraphStyle('sHL', fontSize=9,  textColor=C_WHITE, fontName='Helvetica-Bold', spaceAfter=2)
    sMT = ParagraphStyle('sMT', fontSize=8,  textColor=C_MUTED, fontName='Helvetica',      spaceAfter=1)
    ...
    # Bloco de identificação: Igreja / Congregação / Período
    page_w = landscape(A4)[0] - 3*cm
    id_data = [[
        Paragraph(f'<b>Igreja</b><br/>{pdf_church_name}',     ParagraphStyle('ic', fontSize=8, textColor=C_WHITE, fontName='Helvetica', leading=12)),
        Paragraph(f'<b>Congregacao</b><br/>{pdf_cong_name}',  ParagraphStyle('ic', fontSize=8, textColor=C_WHITE, fontName='Helvetica', leading=12)),
        Paragraph(f'<b>Ano</b><br/>{str(year) if year else "Todos"}', ParagraphStyle('ic', fontSize=8, textColor=C_WHITE, fontName='Helvetica', leading=12)),
        Paragraph(f'<b>Trimestre</b><br/>{pdf_trimester}',    ParagraphStyle('ic', fontSize=8, textColor=C_WHITE, fontName='Helvetica', leading=12)),
        Paragraph(f'<b>Licao</b><br/>{lesson_title if lesson_title else "Todas"}', ParagraphStyle('ic', fontSize=8, textColor=C_WHITE, fontName='Helvetica', leading=12)),
    ]]
...
def membros_pdf():
    ...
    C_ACCENT = colors.HexColor('#2d9cdb')
    ...
    # ── Estilos ────────────────────────────────────────────────────────────
    def ps(name, **kw):
        defaults = dict(fontName='Helvetica', fontSize=9, textColor=C_WHITE,
                        leading=13, spaceAfter=0)
        defaults.update(kw)
        return ParagraphStyle(name, **defaults)

    sTitle    = ps('T',  fontName='Helvetica-Bold', fontSize=20, textColor=C_ACCENT,
                   spaceAfter=4, leading=24)
    sSubtitle = ps('St', fontSize=10, textColor=C_MUTED, spaceAfter=2)
    sChurch   = ps('Ch', fontName='Helvetica-Bold', fontSize=13, textColor=C_WHITE,
                   spaceAfter=2)
    sCong     = ps('Cg', fontSize=9, textColor=C_MUTED, spaceAfter=0)
    sSec      = ps('S',  fontName='Helvetica-Bold', fontSize=11, textColor=C_ACCENT,
                   spaceBefore=14, spaceAfter=6)
    sSub      = ps('Sb', fontName='Helvetica-Bold', fontSize=9, textColor=C_YELLOW,
                   spaceBefore=8, spaceAfter=4)
    sFooter   = ps('F',  fontSize=7, textColor=C_MUTED, alignment=TA_CENTER,
                   spaceBefore=6)
    sCell     = ps('Cl', fontSize=8, textColor=C_WHITE)
    ...
    # ── Coletar dados do scope ─────────────────────────────────────────────
    cong_ids = congregation_ids()

    # Igreja e congregação(ões) visíveis
    congregations = []
    church = None
    if cong_ids:
    ...
    # Cabeçalho
    story.append(Paragraph('EBD – Relatório de Membros', sTitle))
    story.append(Paragraph(f'Gerado em: {datetime.now().strftime("%d/%m/%Y às %H:%M")}', sSubtitle))
    story.append(Spacer(1, .3*cm))

    # Igreja / Congregação
    if church:
        story.append(Paragraph(f'✝ {church.name}', sChurch))
        cong_names = ', '.join(c.name for c in congregations)
        story.append(Paragraph(f'Congregação(ões): {cong_names}', sCong))
    ...
    def make_table(headers, rows, col_widths, alt_col=None):
        """Cria Table ReportLab com tema escuro."""
    ...
    if not classes:
        ...
        for klass in classes:
            ...
            header_text = f'Turma: {klass.name}'
            if klass.congregation:
                header_text += f'  ·  {klass.congregation.name}'
            story.append(Paragraph(header_text, sSub))

            if not students:
                story.append(Paragraph('  Nenhum aluno matriculado.', sCellMuted))
                continue

            # Ordenar por nome
            students_sorted = sorted(students, key=lambda s: s.name)
            headers = ['Nome', 'Idade', 'Aniversário', 'Email']
            cw = [page_w*0.38, page_w*0.10, page_w*0.14, page_w*0.38]
            rows = []
            for s in students_sorted:
                rows.append([
                    s.name,
                    calc_age(s.birth_date),
                    fmt_bday(s.birth_date),
                    s.email or '—',
                ])
    ...
    if not teachers:
        ...
        headers = ['Nome', 'Aniversário', 'Email', 'Telefone']
        cw = [page_w*0.30, page_w*0.13, page_w*0.35, page_w*0.22]
        rows = []
        for t in teachers:
            rows.append([
                t.name,
                fmt_date(t.birth_date),
                t.email or '—',
                t.phone or '—',
            ])
    ...
    # SEÇÃO 3: USUÁRIOS
    # ══════════════════════════════════════════════════════════════════════
    story.append(Paragraph('👤 Usuários do Sistema', sSec))

    # Busca usuários vinculados às congregações visíveis
    users = User.query.filter(
        User.congregation_id.in_(cong_ids)
    ).order_by(User.role, User.name).all()

    # Também inclui CHURCH_ADMIN e SUPERADMIN da igreja (sem congregation_id)
    if current_user.is_superadmin and church:
        church_admins = User.query.filter(
            User.church_id == church.id,
            User.role.in_(('CHURCH_ADMIN', 'SUPERADMIN'))
        ).order_by(User.name).all()
        # Evitar duplicatas
        existing_ids = {u.id for u in users}
        users = list(users) + [u for u in church_admins if u.id not in existing_ids]
    ...
    if not users:
        ...
        rows = []
        for u in users:
            # Tenta pegar telefone do registro de Professor vinculado
            phone = '—'
            if hasattr(u, 'id'):
                teacher_rec = Teacher.query.filter_by(user_id=u.id).first()
                if teacher_rec and teacher_rec.phone:
                    phone = teacher_rec.phone
    ...
    story.append(Paragraph(
        f'EBD – Relatório de Membros  ·  Gerado em {datetime.now().strftime("%d/%m/%Y %H:%M")}',
        sFooter))
