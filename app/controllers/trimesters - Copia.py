...
@trimesters_bp.route('/<int:id>/classes-without-lessons')
@login_required
@admin_required
def classes_without_lessons(id):
    """AJAX: retorna turmas que ainda NÃO têm aulas neste trimestre."""
    t = Trimester.query.get_or_404(id)
    all_classes = scoped(Class).order_by(Class.name).all()
    result = []
    for c in all_classes:
        count = Lesson.query.filter_by(trimester_id=t.id, class_id=c.id).count()
        if count == 0:
            result.append({'id': c.id, 'name': c.name})
    return jsonify({'trimester': t.name, 'classes': result})


@trimesters_bp.route('/<int:id>/generate-for-classes', methods=['POST'])
@login_required
@admin_required
def generate_for_classes(id):
    """Gera aulas somente para as turmas selecionadas (ou todas as sem aulas)."""
    t = Trimester.query.get_or_404(id)
    selected_ids = request.form.getlist('class_ids', type=int)

    if not selected_ids:
        flash('Selecione ao menos uma turma.', 'warning')
        return redirect(url_for('trimesters.index'))

    classes = Class.query.filter(Class.id.in_(selected_ids)).all()

    # Segurança: garantir que as turmas são do scope do usuário
    scoped_ids = {c.id for c in scoped(Class).all()}
    classes = [c for c in classes if c.id in scoped_ids]

    if not classes:
        flash('Nenhuma turma válida selecionada.', 'warning')
        return redirect(url_for('trimesters.index'))
    ...
    db.session.commit()
    turmas_nomes = ', '.join(c.name for c in classes)
    flash(f'✅ {lessons_created} aula(s) gerada(s) para: {turmas_nomes}', 'success')
    return redirect(url_for('trimesters.index'))